...
const LEVELS = [
  ...
  },
  {
    ...
    kicker: "COSMIC CARNIVAL",
    objective: "Crash the asteroid carnival and grind the evil coffee grinder",
    color: "#c8a46a",
  },
] as const;
...
const STORY: Record<Level, Comms[]> = {
  ...
  3: [
    { character: "tiger", name: "TIGERTRON", line: "You beat my fortress? Fine. Enjoy my ASTEROID CARNIVAL!" },
    { character: "lola", name: "LOLA // DEEP COVER", line: "Darling, the prizes are rigged and the cotton candy is hostile." },
    { character: "asa", name: "ASA // SLOW OPS", line: "Boss signature... a giant coffee grinder. I typed... brew." },
    { character: "bean", name: "SPACE BEAN", line: "Bean there. Done that. Time to unplug the espresso of doom." },
  ],
};
...
const CAMPAIGN_NODES = [
  { name: "GRASS VALLEY", type: "HOME", x: 10, y: 80 },
  { name: "AURELIA PRIME", type: "ORBIT", x: 28, y: 62 },
  { name: "THE RATTLEBELT", type: "NEBULA", x: 48, y: 48 },
  { name: "KALDERA IX", type: "LAIR", x: 68, y: 32 },
  { name: "CARNIVAL BELT", type: "FAIR", x: 88, y: 16 },
] as const;
...
const MISSION_INTEL = [
  ...
  { story: "The vector drops into Kaldera IX’s Emberwash Valley. Follow the turquoise river below the ridgeline, break TigerTron’s ground batteries, and reach the attention engine beyond the burning forest.", eta: "SURFACE DROP", signal: "TERRAIN LOCK" },
  { story: "TigerTron’s last trick: a carnival of weaponized asteroids orbiting a giant evil coffee grinder. Ride the midways, dodge the prize bots, and shut down the brew before the galaxy is frothed into foam.", eta: "FAIRGROUND JUMP", signal: "GRINDER PING" },
] as const;
...
const WAVE_PLANS: Record<Level, WaveSpec[]> = {
  ...
  3: [
    { time: 2, name: "TICKET TAKERS", formation: "wedge", units: ["fighter", "fighter", "bot", "fighter", "fighter"] },
    { time: 9, name: "MIDWAY MAYHEM", formation: "pincer", units: ["interceptor", "asteroid", "interceptor", "asteroid", "interceptor"] },
    { time: 17, name: "PRIZE BOT PARADE", formation: "wall", units: ["bot", "bot", "bomber", "bot", "bot"] },
    { time: 26, name: "FERRIS FRAGMENTS", formation: "helix", units: ["asteroid", "fighter", "asteroid", "fighter", "asteroid"] },
    { time: 35, name: "COTTON CANDY CROSS", formation: "cross", units: ["interceptor", "bot", "bomber", "bot", "interceptor", "fighter"] },
    { time: 44, name: "GRINDER ESCORT", formation: "escort", units: ["bomber", "interceptor", "bot", "interceptor", "bomber", "fighter"] },
  ],
};
...
class AudioDirector {
  ...
  tick() {
    ...
    const scales = [
      ...
      // Carnival bounce — brighter, more playful than Kaldera's ember drone.
      [246.9, 311.1, 370, 493.9, 440, 370, 311.1, 277.2],
    ];
    const melody = this.lowShield ? [110, 110, 146.8, 110, 98, 98, 123.5, 82.4] : scales[this.level] || scales[0];
  ...
  playSample(url: string, volume = 0.45) {
  ...
  shot() {
    this.playSample("/arcade/sfx/laser-shot.mp3", 0.38);
    this.tone(720, 0.08, "square", 0.04);
    this.tone(1180, 0.055, "sine", 0.022, 0.025);
  }

  hit() {
    this.playSample("/arcade/sfx/explosion-space.mp3", 0.42);
    this.tone(90, 0.35, "sawtooth", 0.1);
    this.tone(48, 0.5, "square", 0.06);
  }

  explode() {
    this.playSample("/arcade/sfx/explosion-space.mp3", 0.5);
    this.tone(70, 0.4, "sawtooth", 0.1);
  }
...
function createEnemyCraft(level: Level, kind: Exclude<EnemyKind, "asteroid" | "turret" | "boss">) {
  ...
  const hullColors = ["#172334", "#30201d", "#2c1119", "#2a1f14"];
  const hull = new THREE.MeshPhysicalMaterial({ color: hullColors[level] || hullColors[0], metalness: 0.9, roughness: 0.24, clearcoat: 0.42, clearcoatRoughness: 0.2 });
  ...
  const hostile = makeGlowMaterial(level === 2 ? "#ffb02e" : level === 3 ? "#e8b86a" : "#ff315c", 3.2);
...
/** Mission 04 boss — a giant evil coffee grinder with spinning burrs. */
function createCoffeeGrinderBoss() {
  const boss = new THREE.Group();
  const chrome = new THREE.MeshStandardMaterial({ color: "#8a9098", metalness: 0.92, roughness: 0.22, emissive: "#2a1810", emissiveIntensity: 0.35 });
  const copper = new THREE.MeshStandardMaterial({ color: "#b87333", metalness: 0.8, roughness: 0.3, emissive: "#5a2808", emissiveIntensity: 0.4 });
  const bean = new THREE.MeshStandardMaterial({ color: "#3d2214", metalness: 0.2, roughness: 0.7 });

  // Hopper
  const hopper = new THREE.Mesh(new THREE.CylinderGeometry(1.1, 1.6, 1.4, 16), copper);
  hopper.position.y = 2.2;
  boss.add(hopper);
  const lip = new THREE.Mesh(new THREE.TorusGeometry(1.15, 0.12, 8, 24), chrome);
  lip.rotation.x = Math.PI / 2;
  lip.position.y = 2.9;
  boss.add(lip);

  // Main body
  const body = new THREE.Mesh(new THREE.CylinderGeometry(1.7, 1.5, 2.4, 18), chrome);
  body.position.y = 0.4;
  boss.add(body);

  // Glowing angry eyes
  [-0.55, 0.55].forEach((x) => {
    const eye = new THREE.Mesh(new THREE.SphereGeometry(0.28, 12, 10), makeGlowMaterial("#ff3a2e", 5));
    eye.position.set(x, 0.9, 1.45);
    boss.add(eye);
  });

  // Spinning grinder burrs
  const burrs = new THREE.Group();
  for (let i = 0; i < 8; i++) {
    const blade = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.12, 1.6), copper);
  ...
  burrs.position.y = -0.3;
  burrs.userData.spinBurrs = true;
  boss.add(burrs);

  // Coffee bean accents orbiting
  for (let i = 0; i < 6; i++) {
    const beanMesh = new THREE.Mesh(new THREE.SphereGeometry(0.28, 10, 8), bean);
    beanMesh.scale.set(1, 0.65, 1.35);
    const angle = (i / 6) * Math.PI * 2;
    beanMesh.position.set(Math.cos(angle) * 2.4, Math.sin(angle * 2) * 0.4, Math.sin(angle) * 2.4);
    boss.add(beanMesh);
  }

  // Steam ring
  const ring = new THREE.Mesh(new THREE.TorusGeometry(2.5, 0.1, 10, 40), makeGlowMaterial("#e8b86a", 3.5));
  ring.rotation.x = Math.PI / 2;
  ring.position.y = 0.2;
  boss.add(ring);

  const light = new THREE.PointLight("#ff6a28", 48, 24);
  light.position.y = 1;
  boss.add(light);
  return boss;
}

function createBossForLevel(level: Level) {
  return level === 3 ? createCoffeeGrinderBoss() : createTigerBoss();
}

function bossLabel(level: Level) {
  if (level === 3) return "COFFEE GRINDER";
  if (level === 2) return "TIGERTRON";
  return "COMMAND ACE";
}
...
export default function SpaceBeanGame() {
  ...
  useEffect(() => {
    ...
    // Mission 04 carnival lights — neon midway lamps floating among the rocks.
    const carnivalLights = new THREE.Group();
    const carnivalColors = ["#ff4f7b", "#ffd166", "#7dffb3", "#6ad5ff", "#c792ff", "#ff9f1c"];
    for (let i = 0; i < 18; i++) {
      const color = carnivalColors[i % carnivalColors.length];
      const bulb = new THREE.Mesh(
        new THREE.SphereGeometry(0.35 + Math.random() * 0.25, 10, 8),
        new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.92 }),
      );
      bulb.position.set((Math.random() - 0.5) * 70, (Math.random() - 0.5) * 28, -40 - Math.random() * 160);
      carnivalLights.add(bulb);
      const lamp = new THREE.PointLight(color, 8, 18, 2);
      lamp.position.copy(bulb.position);
      carnivalLights.add(lamp);
    }
    // Mini ferris wheel silhouette
    const wheel = new THREE.Group();
    const rim = new THREE.Mesh(
      new THREE.TorusGeometry(7, 0.18, 8, 40),
      new THREE.MeshBasicMaterial({ color: "#ff6ad5", transparent: true, opacity: 0.75 }),
    );
    wheel.add(rim);
    for (let i = 0; i < 8; i++) {
      const spoke = new THREE.Mesh(
        new THREE.BoxGeometry(0.12, 7, 0.12),
        new THREE.MeshBasicMaterial({ color: "#ffd166", transparent: true, opacity: 0.55 }),
      );
      spoke.rotation.z = (i / 8) * Math.PI;
      wheel.add(spoke);
      const gondola = new THREE.Mesh(
        new THREE.BoxGeometry(0.9, 0.7, 0.7),
        new THREE.MeshStandardMaterial({ color: carnivalColors[i % carnivalColors.length], emissive: carnivalColors[i % carnivalColors.length], emissiveIntensity: 0.4 }),
      );
      gondola.position.set(Math.cos((i / 8) * Math.PI * 2) * 7, Math.sin((i / 8) * Math.PI * 2) * 7, 0);
      wheel.add(gondola);
    }
    wheel.position.set(22, 4, -95);
    wheel.userData.spinWheel = true;
    carnivalLights.add(wheel);
    carnivalLights.visible = false;
    scene.add(carnivalLights);
    ...
    const resetMission = () => {
      ...
      planet.visible = lv === 0;
      atmosphere.visible = lv === 0;
      clouds.visible = lv === 0;
      aureliaOrbit.visible = lv === 0;
      asteroidBelt.visible = lv === 1 || lv === 3;
      gasWorld.visible = lv === 1;
      lavaWorld.visible = false;
      kalderaSurface.visible = lv === 2;
      kalderaHorizon.visible = lv === 2;
      carnivalLights.visible = lv === 3;
      ...
      treeCrowns.instanceMatrix.needsUpdate = true;
      if (treeCrowns.instanceColor) treeCrowns.instanceColor.needsUpdate = true;
      const fogByLevel = ["#050916", "#21142b", "#310508", "#1a1208"] as const;
      const bgByLevel = ["#02030a", "#100b1a", "#090104", "#0a0704"] as const;
      const ambSky = ["#88bdff", "#c594df", "#ff7950", "#e8c48a"] as const;
      const ambGnd = ["#100018", "#130d1f", "#180003", "#1a0e04"] as const;
      const keyCol = ["#b8d7ff", "#f2c8ff", "#ffb16d", "#ffd59a"] as const;
      const sunCol = ["#ffe5bd", "#d6a8ff", "#ff321c", "#ffb060"] as const;
      const sunInt = [600, 490, 780, 560];
      scene.fog = new THREE.FogExp2(fogByLevel[lv] || fogByLevel[0], lv === 1 || lv === 3 ? 0.0105 : 0.009);
      scene.background = new THREE.Color(bgByLevel[lv] || bgByLevel[0]);
      renderer.toneMappingExposure = lv === 2 ? 1.04 : lv === 1 || lv === 3 ? 0.98 : 0.94;
      ambient.color.set(ambSky[lv] || ambSky[0]);
      ambient.groundColor.set(ambGnd[lv] || ambGnd[0]);
      ambient.intensity = lv === 1 || lv === 3 ? 1.5 : 1.15;
      key.color.set(keyCol[lv] || keyCol[0]);
      key.intensity = lv === 1 || lv === 3 ? 3.8 : 3.15;
      sun.color.set(sunCol[lv] || sunCol[0]);
      sun.intensity = sunInt[lv] ?? 600;
      const nebulaPalettes = [
        ["#1766c7", "#6a1ea8", "#e94055"],
        ["#8b3b22", "#4e2034", "#d16c2b"],
        ["#ff351f", "#aa102d", "#ff7a18"],
        ["#c47a28", "#7a3a18", "#e8b040"],
      ];
      const nebulaColors = nebulaPalettes[lv] || nebulaPalettes[0];
    ...
    const spawnUnit = (type: EnemyKind, slot: number, squadSize: number, formation: Formation, waveId = -1, asteroidTier = 2) => {
      ...
      const mesh = type === "boss" ? createBossForLevel(level) : type === "asteroid" ? createAsteroid() : type === "turret" ? createGroundTurret() : createEnemyCraft(level, type);
    ...
    const destroyEnemy = (enemy: Enemy, index: number, position: THREE.Vector3) => {
      ...
      if (enemy.boss || enemy.type === "asteroid") audio.explode();
      else audio.hit();
    ...
    const completeMission = () => {
      ...
      if (levelRef.current < 3) {
        ...
        const handoff =
          levelRef.current === 3
            ? { character: "tiger" as const, name: "TIGERTRON", line: "You want an encore? Welcome to my ASTEROID CARNIVAL!" }
            : { character: "bean" as const, name: "SPACE BEAN", line: "One sector safe. Tiger’s trail is still warm." };
        showComms(handoff);
      } else {
        setPhase("victory");
        showComms({ character: "tiger", name: "TIGERTRON", line: "My grinder... my tickets... Fine. I shall knock over something smaller." });
      }
    };
    ...
    const animate = () => {
      ...
      if (carnivalLights.visible) {
        carnivalLights.children.forEach((child, index) => {
          if (child.userData.spinWheel) {
            child.rotation.z += dt * 0.35;
          } else if (child instanceof THREE.Mesh && child.material instanceof THREE.MeshBasicMaterial) {
            child.material.opacity = 0.75 + Math.sin(elapsed * 4 + index) * 0.25;
          } else if (child instanceof THREE.PointLight) {
            child.intensity = 6 + Math.sin(elapsed * 5 + index) * 4;
          }
        });
      }
      ...
      if (playing) {
        ...
        if (waveIndex >= wavePlan.length && !bossSpawned && missionTime > finalWaveTime + 4 && (enemies.length === 0 || missionTime > finalWaveTime + 11)) {
          bossSpawned = true;
          spawnBoss();
          setWaveBanner({ number: wavePlan.length + 1, name: bossLabel(levelRef.current) });
          showComms(
            levelRef.current === 3
              ? { character: "tiger", name: "TIGERTRON // BOSS", line: "Behold! My GIANT EVIL COFFEE GRINDER! It will froth you!" }
              : levelRef.current === 2
      ...
      for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (enemy.boss) {
          ...
          enemy.mesh.rotation.y = Math.sin(enemy.phase * 0.4) * 0.15;
          enemy.mesh.traverse((child) => {
            if (child.userData.spinBurrs) child.rotation.y += dt * 4.5;
          });
        }
        if (!enemy.boss) {
      ...
      if (playing) {
        if (hudAccumulator >= 0.083) {
          ...
          const displayName = bossSpawned ? bossLabel(levelRef.current) : waveIndex > 0 ? plan[waveIndex - 1].name : "STANDBY";
    ...
    <main className={`game game--${phase} ${hud.shield < 28 && phase === "playing" ? "game--danger" : ""} ${flash ? "game--hit" : ""}`}>
      ...
      {phase === "title" && (
        <section className="title-screen">
          ...
          <p className="title-screen__intro">When chaos calls, Little Bean answers. Four missions. One tiny pilot. Chase a catastrophically needy cat from Aurelia to the cosmic coffee carnival.</p>
        ...
        <section className="briefing">
          <div className="briefing__map">
            <div className="campaign-map">
              ...
              <div className="route-line route-line--one" /><div className="route-line route-line--two" /><div className="route-line route-line--three" /><div className="route-line route-line--four" />
          ...
          <div className="briefing__copy">
            ...
            <div className="briefing__intel">
              <div><b>THREAT</b><span>{briefLevel === 0 ? "RISING" : briefLevel === 1 ? "SEVERE" : briefLevel === 2 ? "ABSURD" : "CAFFEINATED"}</span></div>
      ...
      {(phase === "playing" || phase === "paused") && (
        <>
          <section className="hud" aria-live="polite">
            ...
            {hud.progress >= 88 && <div className="boss-meter"><small>{bossLabel(hud.level)}</small><i><b style={{ width: `${hud.boss}%` }} /></i></div>}
          </section>
