...
```bash
npm run build      # -> dist/  (base = /space-bean/)
```

The `build` script sets `--base=/space-bean/` so bundled URLs resolve at that sub-path.
The game's own asset references use `import.meta.env.BASE_URL`, so they resolve at `/`
in dev and `/space-bean/` in the production build automatically.

## Publishing to the Pink Horse Arcade

This game is served at `/space-bean/` on [pinkhorseofwhimsy.com](https://pinkhorseofwhimsy.com).
To rebuild and publish:

```bash
npm install
npm run build
rm -rf ../../site/static/space-bean
cp -R dist/. ../../site/static/space-bean/
```

Then commit the refreshed `site/static/space-bean/`. The arcade card lives in
