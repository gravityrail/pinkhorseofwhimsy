...
Hosted at `/bean-simulator/`. Build with base path:

```bash
npm install
npm run build
rm -rf ../../site/static/bean-simulator && cp -R dist/. ../../site/static/bean-simulator/
```

Commit the refreshed `site/static/bean-simulator/`. Arcade card: `site/src/pages/arcade/index.tsx`.
